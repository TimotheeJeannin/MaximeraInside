...
export function makeShoeGeometry(length, height, width) {
    ...
    taperWidth(geo, length);
    geo.computeVertexNormals();
    return geo;
}

// Narrow the heel and the toe so the shoe reads as a shoe when seen from above.
function taperWidth(geo, length) {
    const pos = geo.attributes.position;
    for (let i = 0; i < pos.count; i++) {
        const t = pos.getX(i) / length + 0.5; // 0 at the heel, 1 at the toe
        const factor = 0.60 + 0.40 * Math.sin(Math.PI * Math.min(Math.max(t, 0), 1) ** 0.75);
