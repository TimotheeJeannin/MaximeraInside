...
print(
    "bbox  x %.4f..%.4f (%.1f cm)  y %.4f..%.4f (%.1f cm)  z %.4f..%.4f (%.1f cm)"
    % (
        min(xs),
        max(xs),
        (max(xs) - min(xs)) * 100,
...
centre = [
    ...
    if abs(xs[i]) < 0.255 and abs(zs[i]) < 0.243 and ys[i] < 0.09
]
print(
    "\n-- low vertices inside the cavity footprint (floor detection), %d pts --"
    % len(centre)
)
